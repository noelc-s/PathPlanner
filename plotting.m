...
if plot_edges
    % scatter(center(:,1),center(:,2),100,repmat([0 1 0],size(center,1),1),'filled');
    % scatter(center(:,1),center(:,2),100,repmat([1 0 0],size(center,1),1),'filled');
    ...
    % x_uncertain_edges = [];
    % y_uncertain_edges = [];
    % for i = 1:size(UncertainEdges,1)
    %     x_uncertain_edges = [x_uncertain_edges [center(NominalEdges(UncertainEdges(i)+1,1)+1,1),center(NominalEdges(UncertainEdges(i)+1,2)+1,1)] NaN];
    %     y_uncertain_edges = [y_uncertain_edges [center(NominalEdges(UncertainEdges(i)+1,1)+1,2),center(NominalEdges(UncertainEdges(i)+1,2)+1,2)] NaN];
    % end
    % plot(x_uncertain_edges, y_uncertain_edges,'c--','linewidth',2);
...
for obs = 1:length(Obstacle_A)
    nom = lcon2vert(Obstacle_A{obs}(:,1:2), Obstacle_b{obs}+result.Planner.buffer);
    inds = convhull(nom);
    nom = nom(inds,:)';
    Obstacle = [nom(1,:) + Obs{1}(obs,1); nom(2,:) + Obs{1}(obs,2)];
    O_p(obs) = patch(Obstacle(1,:),Obstacle(2,:),'r','facealpha',0.0);
end
